...
> [!NOTE]
>
> Nous vous conseillons d'abord de configurer votre environnement de
> développement, de tester l'envoi d'e-mails depuis celui-ci, puis de configurer
> votre environnement de production en conséquence :
>
> 1. [Configuration son environnement de développement](#configuration-son-environnement-de-développement).
> 2. [Envoyer des mails en PHP à l'aide de PHPMailer](#envoyer-des-mails-en-php-à-laide-de-phpmailer).
> 3. [Configurer son environnement de production](#configurer-son-environnement-de-production).
>
...
Mailpit est déjà disponible dans l'environnement de développement fourni pour ce
cours. Vous pouvez donc l'utiliser pour tester l'envoi d'e-mails depuis votre
application PHP à l'aide de Docker Compose.

Suivez les instructions dans le `README.md` du template fourni pour démarrer
Mailpit et accéder à son interface web.
...
L'interface web de Mailpit devrait s'ouvrir, où vous pourrez visualiser les
e-mails capturés :
...
De ce fait, Mailpit doit être en cours d'exécution chaque fois que vous
souhaitez tester l'envoi d'e-mails depuis votre application PHP.

Aucune authentification n'est requise pour utiliser Mailpit, car il est destiné
uniquement au développement local. Vous pouvez donc configurer votre application
PHP pour utiliser Mailpit sans fournir de nom d'utilisateur ou de mot de passe.
...
Vous avez déjà, dans le contexte d'une précédente séance, commandé et configuré
les éléments suivants pour votre projet PHP :
...
Pour acquérir un service de messagerie auprès d'Infomaniak, référez-vous au
contenu
...
Vous pourrez ainsi créer une adresse e-mail personnalisée liée à votre nom de
domaine que vous pourrez utiliser pour envoyer des e-mails via votre application
PHP.
...
6. Copiez le **contenu** du dossier `src` extrait dans le répertoire de votre
   projet PHP dans le dossier `src/PHPMailer/PHPMailer` (notez le double
...
PHP, comme celui que nous avons créé dans le contenu
[Programmation orientée objet (avancé)](../09-programmation-orientee-objet-avance/README.md).

Prenez quelques minutes pour examiner les fichiers de la bibliothèque PHPMailer.
Remarquez-vous des éléments intéressants que nous aurions vus dans les séances
précédents ?
...
Vous pouvez maintenant configurer votre environnement de production pour envoyer
des e-mails en utilisant votre adresse e-mail personnalisée associée à votre nom
de domaine, comme décrit dans la section
[Configurer son environnement de production](#configurer-son-environnement-de-production).

## Conclusion

Au travers de ce dernier contenu, vous avez acquis les compétences nécessaires
pour intégrer la fonctionnalité d'envoi d'e-mails dans vos applications PHP, ce
qui est une compétence essentielle pour de nombreux projets web.
...
Dans de futures cours, vous aurez l'occasion d'utiliser d'autres librairies
tierces pour implémenter d'autres fonctionnalités avancées dans vos applications
PHP.
